      ...
      <div v-for="group in factionsByGroup" :key="group.group" class="mb-10">
        ...
        <div class="space-y-2">
          <router-link
            ...
            class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-4 sm:px-5 py-3 rounded-lg border border-gray-700 bg-gray-800 hover:border-amber-400 hover:bg-gray-700 transition-all cursor-pointer group"
          >
            <div>
              <span class="text-white font-medium group-hover:text-amber-400 transition-colors">{{ faction.name }}</span>
              <span class="text-gray-500 text-sm sm:ml-3">{{ faction.gameName }}</span>
            </div>
